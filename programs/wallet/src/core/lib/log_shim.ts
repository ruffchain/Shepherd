...
class BLogStackHelper {
    static _extractLocation(urlLike: string) {
        // Fail-fast but return locations like '(native)'
        if (urlLike.indexOf(':') === -1) {
            return [urlLike];
        }

        const parts: RegExpExecArray | null = BLOG_LINE_EXP.exec(urlLike.replace(/[\(\)]/g, ''));
        if (parts) {
    ...
    static _parseStackString(stackString: string) {
    ...
    static _getStackString(info: string) {
        let stack;
        try {
...
export class LogShim {
    ...
    constructor(log: any, options: any) {
        this.m_preHeaders = [];
        this.m_postHeaders = [];
        this.m_log = log;
