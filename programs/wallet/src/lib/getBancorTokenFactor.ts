...
export async function getBancorTokenFactor(ctx: IfContext, args: string[]): Promise<IfResult> {
...
export function prnGetBancorTokenFactor(ctx: IfContext, obj: IfResult) {
    if (ctx.sysinfo.verbose) {
        console.log(obj);
    }
    ...
    try {
        ...
        if (objJson.err === 0) {
            console.log('Factor: ', objJson.value);
        } else {
